...
static char   s_szWebRoot[_MAX_PATH];
static HWND   s_ghwnd;
static UINT   s_guAppMsg;
...
BOOL StartHTTP(LPHTTPSERVINFO lpInfo)
{
    ...
    //
    s_ghwnd    = lpInfo->hwnd;
    s_guAppMsg = lpInfo->uMsgApp;
    if (lpInfo->lpRootDir != NULL)
        strcpy(s_szWebRoot, lpInfo->lpRootDir);
    else
        strcpy(s_szWebRoot, "/WebPages");

    //
    // Create the exit signal event object
    //
    s_ghExit = CreateEvent(NULL,        // Security
                         TRUE,        // Manual reset
                         FALSE,        // Initial State
                         NULL);        // Name
    if (s_ghExit == NULL)
    ...
    //
    s_listenSocket = socket(AF_INET, 
                          SOCK_STREAM, 
                          IPPROTO_TCP);
    if (s_listenSocket == INVALID_SOCKET)
    {
        LogWinSockError(s_ghwnd, 
    ...
    //
    nRet = listen(s_listenSocket, SOMAXCONN);
    if (nRet == SOCKET_ERROR)
    {
        LogWinSockError(s_ghwnd,
                         "listen() error",
    ...
    //
    s_gdwListenThread = _beginthreadex(
                            NULL,         // Security
                            0,             // Stack size
...
void StopHTTP()
{
    ...
    //
    nRet = closesocket(s_listenSocket);
...
unsigned __stdcall ListenThread(void *pVoid)
{
    ...
    while(1)
    {
        ...
        socketClient = accept(s_listenSocket,
                              (LPSOCKADDR)&SockAddr,
                              &nLen);
...
int ParseRequest(LPREQUEST lpReq, LPBYTE lpBuf)
{
    ...
    strcpy(lpReq->szFileName, s_szWebRoot);
    if (strlen(cpToken) > 1)
...
void SendFile(LPREQUEST lpReq)
{
    ...
    SendMessage(s_ghwnd,
                s_guAppMsg,
                HTTP_FILEOK_MSG,
                (LPARAM)(LPCSTR)&lpReq->szFileName[strlen(s_szWebRoot)]);
    //
